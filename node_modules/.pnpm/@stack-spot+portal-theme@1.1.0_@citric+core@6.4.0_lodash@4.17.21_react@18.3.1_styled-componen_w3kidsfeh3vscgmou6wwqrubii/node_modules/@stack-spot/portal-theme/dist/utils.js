import { colorPaletteLevel, colorPaletteName, colorSchemeName, theme } from './definition.js';
/**
 ...
 */
export function contrastColor(color, level = 600) {
    if (colorSchemeName.includes(color)) {
        return (level === 'contrastText' || level === 700)
            ? theme.color[color]['300']
            : theme.color[color].contrastText;
    }
 ...
 */
export function valueOf(varname, element = document.body) {
 ...
 */
export function* createColorSequence({ colorOrder, fromColor, fromLevel, levelOrder } = {}) {
 ...
 */
export function* paletteSequence(options) {
 ...
 */
export function withOpacity(colorVariable, opacity) {
 ...
 * @returns
 */
export function listToClass(list) {
