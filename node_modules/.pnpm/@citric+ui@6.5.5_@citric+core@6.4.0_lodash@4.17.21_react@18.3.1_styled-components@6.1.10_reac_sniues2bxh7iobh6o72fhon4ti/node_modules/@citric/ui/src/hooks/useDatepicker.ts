...
const useDatepicker = (props: UseDatepickerProps) => {
  ...
  function setNewDay(day: number) {
    setSelectedDate(setDay(selectedDate, day))
  }

  function setNewMonth(month: number) {
  ...
  function getNameDaysWeek() {
    ...
    const nameDaysWeek = Array.from(Array(7)).map((item, index) =>
      format(addDays(date, index), formatPattern, { locale: handleLocale() }),
    )

    return nameDaysWeek
  }

  function getMonthsName(): Array<{ number: number, name: string, nameFull: string }> {
    const date= new Date()
  ...
  function isSameMonth(day: Date) {
    return isSameMonthDateFns(day, currentDate)
  }

  function isCurrentDate({ day, month, year }: DateProps) {
    const initialDate = props.initialDate || new Date()

    return (
      (day && isSameDay(initialDate, day)) ||
      month === getMonth(initialDate) ||
      year === getYear(initialDate)
    )
  }

  function isSelected({ day, month, year }: DateProps) {
    return (
      (day &&
  ...
  function handleChangeTyping(event: React.ChangeEvent<HTMLInputElement>) {
    if (inputRef.current){
